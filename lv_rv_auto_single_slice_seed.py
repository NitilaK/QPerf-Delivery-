...
def main():
    ...
    folder = r"C:\Users\320289802\OneDrive - Philips\Documents\Cardiac Perfusion\Data\PESA\Data1\S36010"

    # ------------------------------------------------------------
    # STEP 0 — Load DICOMs and group automatically by slice
    # ------------------------------------------------------------
    ...
    for f in dicom_files:
        ...
        if "ImagePositionPatient" in dcm:
            slice_id = round(float(dcm.ImagePositionPatient[2]), 2)
        elif "SliceLocation" in dcm:
            slice_id = round(float(dcm.SliceLocation), 2)
        else:
            slice_id = 0.0

        slice_dict.setdefault(slice_id, []).append(f)

    sorted_slices = sorted(slice_dict.keys())

    print(f"\nDetected {len(sorted_slices)} slices total\n")

    # ------------------------------------------------------------
    # PRINT FRAME RANGE FOR EACH SLICE
    # ------------------------------------------------------------
    print("Slice dynamic ranges:\n")

    for sid in sorted_slices:
        flist = sorted(
            slice_dict[sid],
            key=lambda x: int(''.join(filter(str.isdigit, x)))
        )
        print(f"Slice Z={sid}  ->  Frames {flist[0]} to {flist[-1]}  ({len(flist)} frames)")

    # ============================================================
    # LOOP THROUGH ALL SLICES
    # ============================================================
    global_peak_idx = None
    global_peak_image = None
    global_lv_seed = None
    for idx, slice_id in enumerate(sorted_slices):
        show_debug = (idx == 0)

        print("\n=================================================")
        print(f"Processing slice {idx+1}/{len(sorted_slices)}  Z={slice_id}")
        print("=================================================")
        ...
        # Skip first 10 frames if possible
        sub_files = files[10:] if len(files) > 10 else files

        if len(sub_files) == 0:
            continue

        # --------------------------------------------------------
        # STEP 0.5 — Seed frame auto selection
        # --------------------------------------------------------
        seed_idx = int(0.2 * len(sub_files))
        ...
        t0 = time.time()
        if global_lv_seed is None:

            # Detect LV seed only for slice 1
            lv_seed, lv_ellipses = auto_detect_seed_point(seed_eq, debug=show_debug, return_contours=True)
            global_lv_seed = lv_seed

        else:

            # Reuse LV seed for remaining slices
            lv_seed = global_lv_seed
            print(f"Reusing LV seed point: {lv_seed}")

            # Still detect ellipses so RV detection works
            _, lv_ellipses = auto_detect_seed_point(seed_eq, debug=False, return_contours=True)

        rv_seed = auto_detect_rv_seed_near_lv(lv_seed, lv_ellipses, debug=show_debug)
        ...
        # ---- Extract LV contour and print points ----
        contours_lv, _ = cv2.findContours(lv_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

        if contours_lv:
            lv_contour = max(contours_lv, key=cv2.contourArea)
            lv_points = lv_contour.squeeze()
            print("\nLV contour points (seed frame):")
            print(lv_points)
            print("LV contour shape:", lv_points.shape)

        # ---- Extract Myocardium contour and print points ----
        contours_myo, _ = cv2.findContours(myo_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

        if contours_myo:
            myo_contour = max(contours_myo, key=cv2.contourArea)
            myo_points = myo_contour.squeeze()
            print("\nMyocardium contour points (seed frame):")
            print(myo_points)
            print("MYO contour shape:", myo_points.shape)
        ...
        # ---- Step 4: Generate SI curve ONLY for first usable slice ----
        t0 = time.time()

        if global_peak_idx is None:  # slice 1 (slice 0 is AIF)
            peak_image, global_peak_idx, cycle_pos, all_images = plot_si_curve_and_find_peak(
                folder, from_image, to_image, lv_mask, rv_mask
            )
            global_peak_image = peak_image
            print(f"Global peak detected at frame: {global_peak_image}")

        else:
            # reuse same peak frame index
            peak_image = files[global_peak_idx]
            print(f"Using previously detected peak frame: {peak_image}")

        print(f"[Step 4] Peak frame selection: {time.time() - t0:.2f}s")

        # ---- Step 5: Segmentation on peak frame ----
        t0 = time.time()
        peak_eq, peak_orig = load_dicom_image(os.path.join(folder, peak_image))
        peak_lv_seed, peak_lv_ellipses = auto_detect_seed_point(peak_eq, debug=show_debug, return_contours=True)
        peak_rv_seed = auto_detect_rv_seed_near_lv(peak_lv_seed, peak_lv_ellipses, debug=show_debug)

        min_dist = int(0.05 * max(peak_eq.shape))
        if np.hypot(peak_rv_seed[0]-peak_lv_seed[0], peak_rv_seed[1]-peak_lv_seed[1]) < min_dist:
